// ==================================
// js/roles/cliente.js
// Lógica específica para el dashboard del Cliente (v6 - Optimizado, Correcciones)
// ==================================
console.log('[cliente.js] Módulo cargado. v6');
...
async function populateClienteOrderOptions() {
    console.log("[cliente/populateOrderOptions v6] Poblando opciones...");
    ...
    try {
        ...
        if (tipoBalonSelect) {
            ...
            console.log("[cliente/populateOrderOptions v6] Selector de balón poblado.");
        }
        ...
        if (otrosProductosDiv) {
            ...
            if (!otherProducts.some(p => p.is_available)) {
                ...
                console.log("[cliente/populateOrderOptions v6] Otros productos poblados.");
            }
        }

        // Actualizar precio estimado inicial
        updatePedidoPriceEstimate();
        console.log("[cliente/populateOrderOptions v6] Opciones cargadas OK.");

    } catch (error) {
        console.error("[cliente/populateOrderOptions v6] Error:", error);
...
async function loadClienteHistorial() {
    console.log("[cliente/loadHistorial v6] Cargando historial...");
    ...
    try {
        ...
        console.log(`[cliente/loadHistorial v6] ${history.length} pedidos recibidos.`);
        ...
        console.log("[cliente/loadHistorial v6] Historial cargado OK.");
        
    } catch (error) {
        console.error("[cliente/loadHistorial v6] Error:", error);
...
function downloadReceipt(relativeUrl) {
    ...
    window.open(backendBaseUrl + relativeUrl, '_blank', 'noopener,noreferrer');
}
...
function shareReferralCode() {
    ...
    const code = codeSpan.textContent.trim();
    const whatsappConfig = getGlobalConfigState().whatsapp_number;
...
async function loadClienteProfileData() {
    console.log("[cliente/loadProfileData v6] Cargando datos perfil...");
    
    try {
        // Obtener datos del perfil
        console.log("[cliente/loadProfileData v6] Fetching /users/me...");
        const response = await fetchWithAuth('/users/me');
        ...
        const userData = await response.json();
        console.log("[cliente/loadProfileData v6] Datos recibidos:", userData);
        ...
        console.log("[cliente/loadProfileData v6] Datos de perfil cargados OK.");
        
    } catch (error) {
        console.error("[cliente/loadProfileData v6] Error:", error);
...
export async function setupClienteDashboard() {
    console.log('[cliente.js v6] Configurando Dashboard Cliente...');
    ...
    console.log('[cliente/setup v6] Contenedor del dashboard encontrado.');
    ...
    try {
        ...
        results.forEach((result, index) => {
            if (result.status === 'rejected') {
                ...
                console.error(`[cliente/setup v6] Error al cargar ${modules[index]}:`, result.reason);
            }
        });
        ...
        if (pedidoForm) {
            ...
            console.log('[cliente/setup v6] Listeners formulario de pedido OK.');
        } else {
            console.error("[cliente/setup v6] Formulario #pedido-form no encontrado.");
        }
        ...
        if (fileInput) {
            ...
            console.log('[cliente/setup v6] Listener input de archivo OK.');
        } else {
            console.warn("[cliente/setup v6] Input #payment-proof-input no encontrado.");
        }
        ...
        console.log('[cliente.js v6] Dashboard Cliente configurado OK.');

    } catch (error) {
        console.error('[cliente.js v6] Error FATAL durante setup:', error);
