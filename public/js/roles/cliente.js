// ==================================
// js/roles/cliente.js
// Lógica específica para el dashboard del Cliente (versión corregida)
// ==================================
console.log('[cliente.js] Módulo cargado. Versión corregida');
...
async function populateClienteOrderOptions() {
    console.log("[cliente/populateOrderOptions] Poblando opciones...");
    ...
    try {
        ...
        if (tipoBalonSelect) {
            ...
            console.log("[cliente/populateOrderOptions] Selector de balón poblado.");
        }
        ...
        if (otrosProductosDiv) {
            ...
            if (!otherProducts.some(p => p.is_available)) {
                ...
                console.log("[cliente/populateOrderOptions] Otros productos poblados.");
            }
        }

        // Actualizar precio estimado inicial
        updatePedidoPriceEstimate();
        console.log("[cliente/populateOrderOptions] Opciones cargadas OK.");

    } catch (error) {
        console.error("[cliente/populateOrderOptions] Error:", error);
...
function updatePedidoPriceEstimate() {
    ...
    if (!precioEstimadoSpan) return;
    
    let totalEstimado = 0;
    
    // Cálculo para cilindro seleccionado
    if (tipoBalonSelect && cantidadBalonInput && accionBalonSelect) {
    ...
    if (otrosProductosDiv) {
        otrosProductosDiv.querySelectorAll('input[type="checkbox"]:checked').forEach(cb => {
            ...
            if (quantity > 0) {
                totalEstimado += getCustomerPrice(productId, 'other_product') * quantity;
            }
        });
    }
...
async function handlePedidoSubmit(event) {
    ...
    try {
        ...
        const actionType = formData.get('action_type') || 'exchange';
        const cylinderQuantity = parseInt(formData.get('cylinder_quantity')) || 0;
        
        // Validar dirección de entrega
        const deliveryAddress = formData.get('delivery_address_text');
        if (!deliveryAddress || deliveryAddress.trim() === '') {
            statusDiv.textContent = 'Error: La dirección de entrega es obligatoria.';
        ...
        // Recopilar otros productos correctamente
        const otherItems = [];
        const otherProductsChecked = form.querySelectorAll('#otros-productos-list input[type="checkbox"]:checked');
        
        console.log(`[cliente/handlePedidoSubmit] Productos seleccionados: ${otherProductsChecked.length}`);
        
        otherProductsChecked.forEach(checkbox => {
            const productId = parseInt(checkbox.value);
            const qtyInputId = checkbox.dataset.targetQty || `otro-qty-${productId}`;
            const qtyInput = document.getElementById(qtyInputId);
            
            // Verificar que encontramos el input de cantidad
            if (qtyInput) {
                const quantity = parseInt(qtyInput.value) || 1;
                ...
                if (quantity > 0) {
                    ...
                    console.log(`[cliente/handlePedidoSubmit] Añadido producto ID ${productId}, cantidad ${quantity}`);
                }
            }
        });
        
        // Verificar que hay al menos un producto seleccionado
        if ((cylinderTypeId === null || cylinderTypeId === "" || cylinderQuantity <= 0) && otherItems.length === 0) {
        ...
        const orderData = {
            cylinder_type_id: cylinderTypeId ? parseInt(cylinderTypeId) : null,
            action_type: cylinderTypeId ? actionType : null,
        ...
        console.log("[cliente/handlePedidoSubmit] Datos a enviar:", JSON.stringify(orderData, null, 2));
        ...
        try {
            ...
            result = { message: `Error ${response.status}: No se pudo procesar la respuesta` };
        }
        
        if (!response.ok) {
            throw new Error(result.message || `Error ${response.status}: ${result.errors ? JSON.stringify(result.errors) : 'Error desconocido'}`);
        }
...
async function loadClienteHistorial() {
    console.log("[cliente/loadHistorial] Cargando historial...");
    ...
    try {
        // Obtener historial del cliente con timeout aumentado
        console.log("[cliente/loadHistorial] Iniciando petición a /orders/my-history");
        ...
        console.log(`[cliente/loadHistorial] Datos recibidos:`, result);
        
        // Extraer array de orders del objeto de respuesta
        const history = result.orders || [];
        console.log(`[cliente/loadHistorial] ${history.length} pedidos extraídos.`);
        ...
        if (result.pagination) {
            ...
            console.log(`[cliente/loadHistorial] Datos de paginación:`, pagination);
            
            // Aquí podrías implementar la lógica para mostrar controles de paginación
            // Por ejemplo: Página X de Y, botones de anterior/siguiente, etc.
        }
        
        console.log("[cliente/loadHistorial] Historial cargado OK.");
        
    } catch (error) {
        console.error("[cliente/loadHistorial] Error:", error);
        // Manejar específicamente el error de timeout (AbortError)
        let errorMessage = error.message;
        if (error.name === 'AbortError') {
            errorMessage = "La operación ha excedido el tiempo de espera. Por favor, intente nuevamente.";
        }
        
        // Actualizar la tabla con el mensaje de error
            ...
            finalTableBody.innerHTML = `
                <tr>
                    <td colspan="5" class="error-message">
                        ...
                        <button class="btn btn-sm btn-primary mt-2" onclick="loadClienteHistorial()">
...
async function loadClienteProfileData() {
    console.log("[cliente/loadProfileData] Cargando datos perfil...");
    
    try {
        // Obtener datos del perfil
        console.log("[cliente/loadProfileData] Fetching /users/me...");
        const response = await fetchWithAuth('/users/me');
        ...
        const userData = await response.json();
        console.log("[cliente/loadProfileData] Datos recibidos:", userData);
        ...
        console.log("[cliente/loadProfileData] Datos de perfil cargados OK.");
        
    } catch (error) {
        console.error("[cliente/loadProfileData] Error:", error);
...
export async function setupClienteDashboard() {
    console.log('[cliente.js] Configurando Dashboard Cliente...');

    try {
        // 1. Verificar primero que el contenedor principal existe
        // Esperamos un momento para asegurar que el DOM está completamente cargado
        await new Promise(resolve => setTimeout(resolve, 100));
        
        const dashboardElement = document.querySelector('.cliente-dashboard');
        if (!dashboardElement) {
            console.error("CRITICAL [cliente/setup]: El contenedor principal del dashboard (.cliente-dashboard) no se encontró. No se puede continuar.");
        ...
        console.log('[cliente/setup] Contenedor del dashboard encontrado:', dashboardElement);
        ...
        results.forEach((result, index) => {
            if (result.status === 'rejected') {
                ...
                console.error(`[cliente/setup] Error al cargar ${modules[index]}:`, result.reason);
            }
        });
        ...
        if (pedidoForm) {
            ...
            console.log('[cliente/setup] Listeners formulario de pedido OK.');
        } else {
            console.error("[cliente/setup] Formulario #pedido-form no encontrado.");
        }
        ...
        if (fileInput) {
            ...
            console.log('[cliente/setup] Listener input de archivo OK.');
        } else {
            console.warn("[cliente/setup] Input #payment-proof-input no encontrado.");
        }
        ...
        console.log('[cliente.js] Dashboard Cliente configurado OK.');

    } catch (error) {
        console.error('[cliente.js] Error FATAL durante setup:', error);
        const errorMessage = `Error crítico al configurar panel: ${error.message}.`;
        showGlobalError(errorMessage);
        const mainContent = document.getElementById('main-content');
        if (mainContent) {
            mainContent.innerHTML = `<div class="error-message">${errorMessage}</div>`;
        }
    }
}

// Exponer la función setupClienteDashboard para que pueda ser llamada desde main.js
window.setupClienteDashboard = setupClienteDashboard;

// Auto-inicializar cuando se carga el módulo (solo si está en el rol correcto)
document.addEventListener('DOMContentLoaded', () => {
    // Verificar si estamos en el rol cliente
    const userRoleDisplay = document.getElementById('user-role-display');
    if (userRoleDisplay && userRoleDisplay.textContent.toLowerCase() === 'cliente') {
        console.log('[cliente.js] Auto-inicializando setupClienteDashboard al cargar el módulo');
        // Dar un pequeño tiempo para que el DOM esté completamente listo
        setTimeout(setupClienteDashboard, 200);
    }
});