...
function showAssignedUI(delivery) {
    ...
    if (orderIdEl) orderIdEl.textContent = delivery.order_id;
...
async function loadRepartidorPedidosDisponibles() {
    ...
    try {
        ...
        const response = await fetchWithAuth(`/orders/pending?status=pending_assignment${currentWarehouseId ? '&warehouseId='+currentWarehouseId : ''}`);
        if (!response.ok) throw new Error("Error cargando pedidos disponibles");
        ...
        listDiv.innerHTML = '<p class="error-message">Error al cargar pedidos.</p>';
    }
}
...
async function loadRepartidorHistorial() {
    ...
    const tableBody = document.getElementById('rep-historial-table')?.querySelector('tbody');
    if (!tableBody) return console.error("Tabla de historial repartidor no encontrada.");
    ...
    try {
        ...
        if (!response.ok) throw new Error('Error al cargar historial diario');
        ...
        history.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <td>${formatStatusTag(item.order_status)}</td>
                <td>${item.collection_method ? formatStatusTag(item.collection_method) : '-'}</td>
                <td>${item.amount_collected !== null ? formatCurrency(item.amount_collected) : '-'}</td>
            `;
        });
...
async function loadRepartidorCuadreCaja() {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Error al cargar resumen de caja");
        ...
        resumenDiv.innerHTML = `
            ...
            <p><strong>Cobrado Efectivo:</strong> ${formatCurrency(data.cash_collected)}</p>
            <p><strong>Cobrado Digital:</strong> ${formatCurrency(data.digital_collected)}</p>
            <p><strong>Pendiente de Cobro:</strong> ${formatCurrency(data.pending_amount)} (${data.pending_count || 0} pedidos)</p>
            <hr>
            <p><strong>Total Cobrado/Pendiente:</strong> ${formatCurrency((data.cash_collected || 0) + (data.digital_collected || 0) + (data.pending_amount || 0))}</p>
        `;
        console.log("[repartidor/loadCuadre] Cuadre cargado.");

    } catch (error) {
        console.error("[repartidor/loadCuadre] Error:", error);
        resumenDiv.innerHTML = '<p class="error-message">Error al cargar cuadre.</p>';
    }
}
...
async function startDelivery() {
    ...
    if (!orderIdEl || !startButton || !completeForm) return;
...
async function completeDelivery(event) {
    ...
    if (!form || !errorDiv || !submitButton || !orderIdInput) return;
