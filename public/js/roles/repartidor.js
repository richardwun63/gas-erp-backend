...
import { formatCurrency, formatStatusTag, formatDate } from '../modules/utils.js';
...
let pendingOrdersCheckInterval = null; // Intervalo para verificar pedidos disponibles
let lastErrorShown = 0; // Timestamp del último error mostrado (para evitar spam)
...
async function checkRepartidorAssignment() {
    ...
    try {
        ...
        // Solo mostrar error en el banner si es un error diferente a 404
        if (statusBannerText && !error.message.includes('404')) 
            statusBannerText.textContent = `Error (${error.message})`;
            
        // No mostrar UI de espera si hay un error general (no 404),
        // para evitar perder la visualización actual
...
function showAssignedUI(delivery) {
    ...
    if (delivery.departed_at) {
        // Si ya está en ruta, mostrar el formulario de completar
        startButton.style.display = 'none';
        completeForm.style.display = 'block';
    } else {
        // Si solo está asignado, mostrar botón de iniciar
...
async function loadRepartidorPedidosDisponibles() {
    ...
    try {
        ...
        // Añadir timeout más extenso (15 segundos) y tamaño de página pequeño para reducir carga
        endpoint += `&limit=10`;
        ...
        pedidos.forEach(pedido => {
            ...
            try {
                ...
                formattedDate = orderDate.toLocaleString('es-PE', {
                    day: '2-digit',
                    month: '2-digit',
                    hour: '2-digit',
                    minute: '2-digit'
                ...
                formattedDate = pedido.order_date;
            }
        ...
        // Prevenir múltiples mensajes de error en poco tiempo
        const now = Date.now();
        if (now - lastErrorShown > 60000) { // Solo mostrar error cada minuto como máximo
            lastErrorShown = now;
            
            if (error.name === 'AbortError') {
                listDiv.innerHTML = `<p class="error-message">Error al cargar pedidos: Tiempo de espera agotado. Intentando nuevamente...</p>`;
            } else {
                listDiv.innerHTML = `<p class="error-message">Error al cargar pedidos: ${error.message}</p>`;
            }
        } else {
            // Si mostramos error recientemente, solo actualizar el mensaje sin notificar
            listDiv.innerHTML = `<p>Cargando pedidos disponibles...</p>`;
        }
    }
}
...
async function takeOrder(orderId) {
    ...
    try {
        ...
        loadRepartidorPedidosDisponibles();
...
async function loadRepartidorCuadreCaja(date = null) {
    ...
    try {
        ...
        // Si existe, actualizar el selector de fecha
        if (dateSelector && !date) {
            dateSelector.value = reportDate;
        }
        
        // Obtener datos del reporte de reconciliación para el repartidor
        const response = await fetchWithAuth(`/reports/reconciliation/detail?repartidorId=${user.id}&date=${reportDate}`);
        ...
        // Si no hay entregas para la fecha seleccionada
        if (!data.deliveries || data.deliveries.length === 0) {
            resumenDiv.innerHTML = `<p>No hay datos para ${formatDate ? formatDate(reportDate) : reportDate}.</p>`;
            return;
        }
        ...
        resumenDiv.innerHTML = `
            <p><strong>Fecha:</strong> ${formatDate ? formatDate(data.date) : data.date}</p>
            <p><strong>Entregas Completadas:</strong> ${totals.deliveries || 0}</p>
            <p><strong>Cobrado Efectivo:</strong> ${formatCurrency ? formatCurrency(totals.cash || 0) : `S/ ${(totals.cash || 0).toFixed(2)}`}</p>
            <p><strong>Cobrado Digital:</strong> ${formatCurrency ? formatCurrency(totals.digital || 0) : `S/ ${(totals.digital || 0).toFixed(2)}`}</p>
            <p><strong>Pendiente de Cobro:</strong> ${formatCurrency ? formatCurrency(totals.pending || 0) : `S/ ${(totals.pending || 0).toFixed(2)}`}</p>
            <hr>
            <p><strong>Total:</strong> ${formatCurrency ? formatCurrency(totals.total || 0) : `S/ ${(totals.total || 0).toFixed(2)}`}</p>
        `;
        console.log("[repartidor/loadCuadre] Cuadre cargado para fecha:", reportDate);
...
function handleDateChange(event) {
    ...
    console.log("[repartidor/handleDateChange] Fecha seleccionada:", selectedDate);
    loadRepartidorCuadreCaja(selectedDate);
}
// Exponer globalmente para usar en onchange
...
async function viewDetailedCashReport() {
    ...
    // Obtener la fecha seleccionada del selector o usar la actual
    const dateSelector = document.getElementById('rep-date-selector');
    const reportDate = dateSelector?.value || new Date().toISOString().split('T')[0];
    ...
    try {
        ...
        let tableHtml = '<p>No hay entregas registradas para esta fecha.</p>';
        if (data.deliveries && data.deliveries.length > 0) {
            ...
            data.deliveries.forEach(d => {
                tableHtml += `<tr>
                    ...
                    <td>${d.collection_method ? (formatStatusTag ? formatStatusTag(d.collection_method) : d.collection_method) : '-'}</td>
                    <td>${d.amount_collected !== null ? (formatCurrency ? formatCurrency(d.amount_collected) : `S/ ${d.amount_collected.toFixed(2)}`) : '-'}</td>
                    <td style="white-space: normal;">${d.notes || ''} ${d.has_issue ? '<strong class="text-danger">(Problema)</strong>' : ''}</td>
                </tr>`;
            });
        ...
        modalContent.innerHTML = `
            ...
            <h2>Detalle de Caja - ${formatDate ? formatDate(data.date) : data.date}</h2>
            ${tableHtml}
            <hr>
...
/** Verifica pedidos pendientes periódicamente */
function startPendingOrdersCheck() {
    if (pendingOrdersCheckInterval) clearInterval(pendingOrdersCheckInterval);
    
    // Cargar inmediatamente
    loadRepartidorPedidosDisponibles();
    
    // Luego configurar intervalo cada 20 segundos
    pendingOrdersCheckInterval = setInterval(() => {
        loadRepartidorPedidosDisponibles();
    }, 20000);
    
    console.log("[repartidor/startPendingOrdersCheck] Verificación periódica de pedidos iniciada.");
}
...
export async function setupRepartidorDashboard() {
    ...
    try {
        ...
        // Iniciar verificación periódica de pedidos disponibles
    ...
    // Listener para selector de fecha (para filtrar cuadre de caja)
    const dateSelector = document.getElementById('rep-date-selector');
    if (dateSelector) {
...
export function cleanupRepartidorDashboard() {
   ...
   assignedOrderTimerInterval = null;
   pendingOrdersCheckInterval = null;
   assignedOrderStartTime = null;
