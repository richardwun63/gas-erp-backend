// ==================================
// js/roles/gerente.js
// Lógica específica para el dashboard del Gerente
// ==================================
...
async function loadGerenteKPIs() {
    ...
    try {
        ...
        // Construir HTML
        let kpiHtml = `
            <div class="kpi-card"><h4><i class="fas fa-dollar-sign"></i> Ventas Mes</h4><p class="blue">${formatCurrency(data.sales?.month_total)}</p><small>(${data.sales?.month_orders || 0} pedidos)</small></div>
            <div class="kpi-card"><h4><i class="fas fa-chart-line"></i> Crecim. vs Mes Ant.</h4><p class="${data.sales?.growth_percentage >= 0 ? 'green' : 'red'}"><i class="fas ${data.sales?.growth_percentage >= 0 ? 'fa-arrow-up' : 'fa-arrow-down'}"></i> ${parseFloat(data.sales?.growth_percentage || 0).toFixed(1)}%</p><small>${data.sales?.last_month_total > 0 ? `(vs ${formatCurrency(data.sales.last_month_total)})` : '(Sin mes anterior)'}</small></div>
            <div class="kpi-card"><h4><i class="fas fa-users"></i> Clientes Activos (30d)</h4><p>${data.customers?.active || 0}</p><small>${data.customers?.active_percentage ? `(${data.customers.active_percentage}%)` : ''}</small></div>
            <div class="kpi-card"><h4><i class="fas fa-inbox"></i> Pedidos Pendientes</h4><p class="orange">${data.operations?.pending_orders || 0}</p></div>
            <div class="kpi-card"><h4><i class="fas fa-hand-holding-usd"></i> Cobro Pendiente</h4><p class="red">${formatCurrency(data.operations?.pending_amount)}</p><small>(${data.operations?.pending_payments || 0} pedidos)</small></div>
            <div class="kpi-card"><h4><i class="fas fa-exclamation-triangle"></i> Stock Bajo</h4><p class="red">${data.operations?.low_stock_items || 0}</p><small>items</small></div>
        `;
    ...
    try {
        ...
        monthlySalesChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    tooltip: { callbacks: { label: (context) => `Ventas: ${formatCurrency(context.parsed.y)}` } }
                }
            }
        });
    } catch (error) {
...
async function loadGerenteClientes() {
    ...
    if (!tableBody) return console.error('Tabla #clientes-table tbody no encontrada.');
    ...
    try {
        ...
        clientes.forEach(cliente => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-info btn-sm" title="Ver Detalles"><i class="fas fa-eye"></i></button>
                    <button class="btn btn-warning btn-sm" title="Editar Precios"><i class="fas fa-dollar-sign"></i></button>
                </td>`;
            
            row.querySelector('.btn-info').addEventListener('click', () => viewClientDetails(cliente.user_id));
            row.querySelector('.btn-warning').addEventListener('click', () => openEditCustomerPricesModal(cliente.user_id));
    ...
    try {
        ...
        if (cust.special_prices && cust.special_prices.length > 0) {
            ...
            cust.special_prices.forEach(p => {
                pricesHtml += `<li>${p.cylinder_name}: ${formatCurrency(p.price_exchange)} (Intercambio)</li>`;
            });
            pricesHtml += '</ul>';
        }
        ...
        modalContent.innerHTML = `
            ...
            <div class="grid-container-responsive">
                ...
                <div><strong>Cód. Referido:</strong> ${cust.details?.referral_code || '-'}</div>
                <div><strong>Referido por:</strong> ${cust.details?.referred_by_code || '-'}</div>
...
async function openEditCustomerPricesModal(userId) {
    ...
    try {
        ...
        if (!cylindersRes.ok) throw new Error("Error cargando tipos de balón");
        if (!customerRes.ok) throw new Error("Error cargando datos del cliente");
        ...
        cylinderTypes.forEach(cyl => {
            if (cyl.is_available) {
                ...
                const currentPriceValue = currentSpecial ? currentSpecial.price_exchange : '';
                formHtml += `
        ...
        formHtml += `
                ...
                <button type="button" class="btn btn-danger" style="margin-left: 10px;" onclick="window.clearCustomPrices(${userId})">
...
async function clearCustomPrices(userId) {
    ...
    try {
        ...
        const result = await response.json();
        if (!response.ok && response.status !== 204) throw new Error(result.message || `Error ${response.status}`);
    ...
    try {
        ...
        users.forEach(emp => {
            ...
            row.innerHTML = `
                ...
                <td>${statusTagHTML}</td>
                <td>
                    <button class="btn btn-secondary btn-sm" title="Editar Datos"><i class="fas fa-edit"></i></button>
                    ${emp.role_name === 'repartidor' ? `<button class="btn btn-info btn-sm" title="Editar Horario">
                        <i class="fas fa-calendar-alt"></i></button>` : ''}
                    <button class="btn ${emp.is_active ? 'btn-warning' : 'btn-success'} btn-sm" title="${emp.is_active ? 'Desactivar' : 'Activar'}">
                        <i class="fas ${emp.is_active ? 'fa-user-slash' : 'fa-user-check'}"></i></button>
                    <button class="btn btn-danger btn-sm" title="Eliminar Empleado"><i class="fas fa-trash"></i></button>
                </td>`;

            row.querySelector('.btn-secondary').addEventListener('click', () => openEditUserModal(emp.user_id));
            const scheduleBtn = row.querySelector('.btn-info');
            if(scheduleBtn) scheduleBtn.addEventListener('click', () => openEditScheduleModal(emp.user_id));
            row.querySelector(`.btn-${emp.is_active ? 'warning' : 'success'}`).addEventListener('click', () => 
                toggleUserStatus(emp.user_id, !emp.is_active));
            row.querySelector('.btn-danger').addEventListener('click', () => deleteEmployee(emp.user_id));
...
async function openEditScheduleModal(userId) {
    ...
    try {
        const response = await fetchWithAuth(`/users/${userId}`);
        if (!response.ok) throw new Error("Error al cargar datos del repartidor");
...
async function loadGerenteInventarioData() {
    ...
    if (!warehouseId) {
        document.getElementById('stock-list-balones')?.innerHTML = '<li>Seleccione un almacén</li>';
        document.getElementById('stock-list-otros')?.innerHTML = '<li>Seleccione un almacén</li>';
    ...
    try {
        ...
        if (!stockResponse.ok) throw new Error("Error cargando stock");
        ...
        loans.forEach(loan => {
            ...
            li.innerHTML = `
                ...
                <div class="item-actions">
                    <button class="btn btn-success btn-sm" onclick="window.returnSupplierLoanHandler(${loan.loan_id})">
...
async function populatePrestamoProveedorForm() {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Error al cargar tipos de cilindro");
        ...
        if (form) {
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (!stockCheckRes.ok) throw new Error("Error verificando stock disponible");
...
async function manageOtherProducts() {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Error al cargar otros productos");
...
async function runReport(reportType) {
    ...
    try {
        ...
        switch(reportType) {
            case 'ventas-diarias':
                responseData = await fetchReportData('/reports/daily-sales', {
                    start_date: document.getElementById('report-start-date')?.value,
                    end_date: document.getElementById('report-end-date')?.value
                });
                reportHtml = displayReportData('Reporte de Ventas Diarias', responseData, {
                ...
                if (responseData && responseData.length > 0) {
                    ...
                    setTimeout(() => setupDailySalesChart(responseData), 100);
                }
                break;
            ...
            case 'ventas-productos':
                ...
                reportHtml = displayReportData('Ventas por Producto', responseData, {
                ...
                if (responseData && responseData.length > 0) {
                    ...
                    setTimeout(() => setupProductSalesChart(responseData), 100);
                }
                break;
            ...
            case 'ranking-clientes':
                ...
                reportHtml = displayReportData('Ranking de Clientes', responseData, {
                    ...
                    formatter: (item, index) => [
                        ...
                        item.order_count || '0',
                        formatCurrency(item.total_amount),
                        item.last_purchase_date_formatted || 'N/A'
                    ]
                });
                break;
            ...
            case 'stock-almacenes':
                ...
                reportHtml = displayReportData('Stock por Almacén', responseData, {
                    ...
                    formatter: (item) => [
                        ...
                        item.status_name === 'full' ? 'Lleno' : 
                            ...
                            (item.status_name === 'damaged' ? 'Dañado' : item.status_name))
                    ],
                    rowClassFn: (item) => {
            ...
            case 'historial-ajustes':
                ...
                reportHtml = displayReportData('Historial de Ajustes de Inventario', responseData, {
...
function setupDailySalesChart(data) {
    ...
    const salesData = data.map(d => parseFloat(d.total_amount) || 0);
    const orderData = data.map(d => parseInt(d.order_count) || 0);
...
function setupProductSalesChart(data) {
    ...
    const labels = topProducts.map(d => d.product_name);
...
async function openEditWarehouseModal(warehouseId) {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Error al cargar datos del almacén");
...
// Inicialización
async function initGerenteModule() {
    ...
    try {
        ...
        if (!currentUser || currentUser.role_name !== 'gerente') {
...
export default {
    init: initGerenteModule,
    loadKPIs: loadGerenteKPIs,
    loadClientes: loadGerenteClientes,
    loadEmpleados: loadGerenteUsers,
    loadInventario: loadGerenteInventarioData,
    loadReportes: loadGerenteReportes,
    loadConfig: loadGerenteConfig
};