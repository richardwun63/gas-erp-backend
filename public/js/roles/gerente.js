// ==================================
// js/roles/gerente.js
// Lógica específica para el dashboard del Gerente (CORREGIDO)
// ==================================
...
async function loadGerenteClientes() {
    ...
    try {
        ...
        const result = await response.json();
        // IMPORTANTE: Acceder correctamente a la propiedad 'customers' en la respuesta
        ...
        tableBody.innerHTML = `<tr><td colspan="6"><div class="error-message">Error al cargar clientes: ${error.message}</div></td></tr>`;
    }
}
...
async function viewClientDetails(userId) {
    ...
    try {
        ...
        const result = await response.json();
        const cust = result.data || result; // Adaptar a ambos formatos de respuesta
...
async function openEditCustomerPricesModal(userId) {
    ...
    try {
        ...
        const customerRawData = await customerRes.json();
        
        // Manejar el formato de respuesta, que puede tener la data en .data o directamente
        const customerData = customerRawData.data || customerRawData;
...
async function loadGerenteUsers() {
    ...
    try {
        const response = await fetchWithAuth('/users?excludeRole=cliente');
        if (!response.ok) {
            const errData = await response.json().catch(()=>({}));
        ...
        const result = await response.json();
        // CORREGIDO: Asegurarse de que accedemos a la propiedad 'employees' en la respuesta
        ...
        // CORREGIDO: Ahora itera sobre el array de empleados
        employees.forEach(emp => {
            ...
            row.innerHTML = `
                ...
                <td>${emp.phone_number_primary || '-'}</td>
                <td>${statusTagHTML}</td>
                <td>
                    <button class="btn btn-info btn-sm" title="Ver" onclick="viewUserDetails(${emp.user_id})">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-secondary btn-sm" title="Editar" onclick="openEditUserModal(${emp.user_id})">
                        <i class="fas fa-edit"></i>
                    </button>
                    ${emp.role_name === 'repartidor' ? 
                        `<button class="btn btn-primary btn-sm" title="Horario" onclick="openEditScheduleModal(${emp.user_id})">
                            <i class="fas fa-clock"></i>
                        </button>` : ''
                    }
                    <button class="btn ${emp.is_active ? 'btn-warning' : 'btn-success'} btn-sm" 
                            title="${emp.is_active ? 'Desactivar' : 'Activar'}" 
                            onclick="toggleUserStatus(${emp.user_id}, ${!emp.is_active})">
                        <i class="fas ${emp.is_active ? 'fa-ban' : 'fa-check'}"></i>
                    </button>
                    <button class="btn btn-danger btn-sm" title="Eliminar" onclick="deleteEmployee(${emp.user_id})">
...
async function loadGerenteInventarioData() {
    ...
    try {
        ...
        // Para debug: mostrar los datos que vienen de la API
        console.log("Datos recibidos de inventario:", stockData);

        // Limpiar listas
        balonesList.innerHTML = '';
        otrosList.innerHTML = '';

        // Verificar específicamente si hay cylinders y si es un array
        if (stockData.cylinders && Array.isArray(stockData.cylinders) && stockData.cylinders.length > 0) {
            stockData.cylinders.forEach(cyl => {
                // Verificar que cyl tenga las propiedades esperadas
                console.log("Procesando cilindro:", cyl);
                
                const fullQty = parseInt(cyl.full_qty || 0);
                const emptyQty = parseInt(cyl.empty_qty || 0);
                const damagedQty = parseInt(cyl.damaged_qty || 0);
                
                if (fullQty > 0) {
                    const li = document.createElement('li');
                    li.innerHTML = `<span class="item-name">${cyl.name}</span> <span class="item-status">Lleno</span> <span class="item-quantity">${fullQty}</span>`;
                    balonesList.appendChild(li);
                }
                
                if (emptyQty > 0) {
                    const li = document.createElement('li');
                    li.innerHTML = `<span class="item-name">${cyl.name}</span> <span class="item-status">Vacío</span> <span class="item-quantity">${emptyQty}</span>`;
                    balonesList.appendChild(li);
                }
                
                if (damagedQty > 0) {
                    const li = document.createElement('li');
                    li.innerHTML = `<span class="item-name">${cyl.name}</span> <span class="item-status text-danger">Dañado</span> <span class="item-quantity">${damagedQty}</span>`;
                    balonesList.appendChild(li);
                }
            });
        } else {
            // Comprobar si cylinders está presente pero vacío o con formato incorrecto
            if (stockData.cylinders !== undefined) {
                console.warn("cylinders está presente pero no es un array o está vacío:", stockData.cylinders);
            } else {
                console.warn("cylinders no está presente en la respuesta");
            }
            balonesList.innerHTML = '<li>No hay balones registrados.</li>';
        }

        // Verificar si hay otherProducts y si es un array
        if (stockData.otherProducts && Array.isArray(stockData.otherProducts) && stockData.otherProducts.length > 0) {
            stockData.otherProducts.forEach(prod => {
                const stockQty = parseInt(prod.stock_qty || 0);
                if (stockQty > 0) {
                    const li = document.createElement('li');
                    li.innerHTML = `<span class="item-name">${prod.name}</span> <span class="item-status">Disponible</span> <span class="item-quantity">${stockQty}</span>`;
        ...
        // También verificar si hay datos en el inventario usando la estructura alternativa
        // (por si el backend devuelve un formato diferente)
        if (balonesList.children.length === 0 && stockData.items) {
            const cylinderItems = stockData.items.filter(item => item.item_type === 'cylinder');
            if (cylinderItems.length > 0) {
                cylinderItems.forEach(item => {
                    const li = document.createElement('li');
                    li.innerHTML = `<span class="item-name">${item.name}</span> <span class="item-status">${item.status}</span> <span class="item-quantity">${item.quantity}</span>`;
                    balonesList.appendChild(li);
                });
            } else {
                balonesList.innerHTML = '<li>No hay balones registrados (formato alternativo).</li>';
            }
        }
...
async function populatePrestamoProveedorForm() {
    ...
    try {
        // Intento con timeout reducido para este endpoint problemático
        const response = await fetchWithAuth('/products/cylinders', {}, 1); // Solo 1 reintento
        if (!response.ok) {
            const errData = await response.json().catch(()=>({}));
        ...
        // Cargar opciones de respaldo si falla la petición
        select.innerHTML = `
            <option value="" disabled selected>-- Error al cargar tipos --</option>
            <option value="1">5kg</option>
            <option value="2">10kg</option>
            <option value="3">15kg</option>
            <option value="4">45kg</option>
        `;
    }
}
...
async function loadGerenteConfig() {
    ...
    if (!tabConfiguracion) {
        console.error('[gerente/loadConfig] Tab #tab-configuracion no encontrado.');
        return;
    }
    
    // Verificar si el contenedor de config existe, si no, crearlo
    let configContainer = tabConfiguracion.querySelector('#config-container');
    if (!configContainer) {
        console.log('[gerente/loadConfig] Creando contenedor de configuración...');
        configContainer = document.createElement('div');
        configContainer.id = 'config-container';
        tabConfiguracion.appendChild(configContainer);
    }

    // Crear el formulario general si no existe
    let form = tabConfiguracion.querySelector('#config-general-form');
    if (!form) {
        console.log('[gerente/loadConfig] Creando formulario de configuración general...');
            ...
            <div class="card">
                ...
                <form id="config-general-form">
                    ...
                    <div class="input-group">
                        ...
                        <input type="tel" id="whatsapp_number" name="whatsapp_number" placeholder="Ej. 51999999999">
                    </div>
                    <div class="input-group">
                        <label for="benefits_description">Descripción de Beneficios:</label>
                        <textarea id="benefits_description" name="benefits_description" rows="3"></textarea>
                    </div>
                    <div class="input-group">
                        <label for="points_per_sol">Puntos por S/:</label>
                        <input type="number" id="points_per_sol" name="points_per_sol" step="0.01" min="0">
                    </div>
                    <div class="input-group">
                        <label for="points_min_redeem">Puntos Mínimos para Canjear:</label>
                        <input type="number" id="points_min_redeem" name="points_min_redeem" min="0">
                    </div>
                    <div class="input-group">
                        <label for="points_discount_value">Valor del Descuento (S/):</label>
                        <input type="number" id="points_discount_value" name="points_discount_value" step="0.10" min="0">
                    </div>
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Guardar Configuración</button>
                    <div id="config-form-error" class="error-message"></div>
                </form>
            </div>
        `;
        form = document.getElementById('config-general-form');
    }
    ...
    try {
        ...
        if (form) {
            ...
            if (elements['company_name']) elements['company_name'].value = config.company_name?.value || '';
            if (elements['address']) elements['address'].value = config.company_address?.value || '';
            if (elements['whatsapp_number']) elements['whatsapp_number'].value = config.whatsapp_number?.value || '';
            if (elements['benefits_description']) elements['benefits_description'].value = config.benefits_description?.value || '';
            if (elements['points_min_redeem']) elements['points_min_redeem'].value = config.points_min_redeem?.value || '';
            if (elements['points_discount_value']) elements['points_discount_value'].value = config.points_discount_value?.value || '';
            if (elements['points_per_sol']) elements['points_per_sol'].value = config.points_per_sol?.value || '';
...
async function handleSaveConfig(event) {
    ...
    try {
        ...
        // Convertir a objetos con value y type
        for (const [key, value] of formData.entries()) {
            if (value.trim() !== '') { // Solo enviar valores no vacíos
                if (['points_min_redeem', 'points_for_referral'].includes(key)) {
                    dataToSend[key] = parseInt(value);
                } else if (['points_discount_value', 'points_per_sol'].includes(key)) {
...
async function manageWarehouses() {
    // Esta función simplemente muestra/oculta la sección de almacenes
    // El contenido ya está cargado por loadWarehouses()
    const warehousesSection = document.querySelector('#tab-configuracion .card:nth-child(1)');
    if (warehousesSection) {
        warehousesSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
    } else {
        showGlobalError("Sección de almacenes no encontrada");
    }
}
window.manageWarehouses = manageWarehouses;
...
// Implementar las funciones faltantes que causaban errores
async function viewUserDetails(userId) {
    ...
    try {
        ...
        if (!response.ok) {
            const errData = await response.json().catch(()=>({}));
            throw new Error(errData.message || `Error ${response.status} al cargar detalles del usuario`);
        }
        
        const user = await response.json();

        // Formatear información de horario si existe
        let scheduleInfo = 'No definido';
        if (user.schedule_start && user.schedule_end) {
            scheduleInfo = `${user.schedule_start} a ${user.schedule_end}`;
        }

        modalContent.innerHTML = `
            <span class="close-button" onclick="closeModal('${modalId}')">×</span>
            <h2>Detalle Empleado: ${user.full_name || 'N/A'}</h2>
            <div class="grid-container-responsive">
                <div><strong>ID:</strong> ${user.user_id}</div>
                <div><strong>Username:</strong> ${user.username || '-'}</div>
                <div><strong>Rol:</strong> ${user.role_name || '-'}</div>
                <div><strong>Teléfono 1:</strong> ${user.phone_number_primary || '-'}</div>
                <div><strong>Teléfono 2:</strong> ${user.phone_number_secondary || '-'}</div>
                <div><strong>Email:</strong> ${user.email || '-'}</div>
                <div><strong>Estado:</strong> ${formatStatusTag(user.is_active ? 'activo' : 'inactivo')}</div>
                <div><strong>Almacén Asignado:</strong> ${user.warehouse_name || 'Ninguno'}</div>
                ${user.role_name === 'repartidor' ? `<div><strong>Horario:</strong> ${scheduleInfo}</div>` : ''}
            </div>
            <hr>
            <button type="button" class="btn btn-secondary" onclick="closeModal('${modalId}')">Cerrar</button>
            <button type="button" class="btn btn-secondary" onclick="openEditUserModal(${userId})">
                <i class="fas fa-edit"></i> Editar Datos
            </button>
            ${user.role_name === 'repartidor' ? `
                <button type="button" class="btn btn-primary" onclick="openEditScheduleModal(${userId})">
                    <i class="fas fa-clock"></i> Editar Horario
                </button>` : ''}
        `;
    } catch (error) {
        console.error(`[gerente/viewUserDetails] Error:`, error);
        modalContent.innerHTML = `
            <span class="close-button" onclick="closeModal('${modalId}')">×</span>
...
// Función para manejar los horarios de todos los repartidores
async function manageRepartidorSchedules() {
    const modalId = 'view-details-modal';
    const modal = document.getElementById(modalId);
    const modalContent = modal?.querySelector('.modal-content');
    if (!modal || !modalContent) return showGlobalError("Error: Modal de horarios no encontrado.");

    modalContent.innerHTML = '<div class="loading-placeholder"><i class="fas fa-spinner fa-spin"></i> Cargando horarios de repartidores...</div>';
    openModal(modalId);

    try {
        const response = await fetchWithAuth('/users?role=repartidor');
        if (!response.ok) {
            const errData = await response.json().catch(()=>({}));
            throw new Error(errData.message || `Error ${response.status} al cargar lista de repartidores`);
        }
        ...
        let tableHtml = `
            <h2><i class="fas fa-clock"></i> Horarios de Repartidores</h2>
            <div class="table-responsive">
                <table class="table table-sm">
                    <thead>
                        <tr>
        ...
        if (repartidores.length === 0) {
            tableHtml += '<tr><td colspan="5" style="text-align: center;">No hay repartidores registrados.</td></tr>';
        } else {
            repartidores.forEach(rep => {
                const scheduleInfo = (rep.schedule_start && rep.schedule_end) ? 
                    `${rep.schedule_start} a ${rep.schedule_end}` : 
                    'No definido';
                
                tableHtml += `
                    <tr>
                        <td>${rep.user_id}</td>
                        <td>${rep.full_name || 'N/A'}</td>
                        <td>${scheduleInfo}</td>
                        <td>${formatStatusTag(rep.is_active ? 'activo' : 'inactivo')}</td>
                        <td>
                            <button class="btn btn-primary btn-sm" onclick="openEditScheduleModal(${rep.user_id})">
        ...
        console.error(`[gerente/manageRepartidorSchedules] Error:`, error);
        modalContent.innerHTML = `
            <span class="close-button" onclick="closeModal('${modalId}')">×</span>
            <h2>Horarios de Repartidores</h2>
            <p class="error-message">Error al cargar horarios: ${error.message}</p>
            <button type="button" class="btn btn-secondary" onclick="closeModal('${modalId}')">Cerrar</button>
        `;
    }
}
window.manageRepartidorSchedules = manageRepartidorSchedules;

// Funciones para guardar configuración de whatsapp
function saveWhatsAppConfig() {
    ...
    if (numero && !/^\d+$/.test(numero)) {
        alert("Por favor ingrese solo números en el número de WhatsApp.");
        return;
    }
    
    // Usar el mismo formulario general para guardar esta configuración específica
    const generalForm = document.getElementById('config-general-form');
    if (generalForm) {
        const whatsappField = generalForm.elements['whatsapp_number'];
        if (whatsappField) {
            whatsappField.value = numero;
            // Simular envío del formulario
            handleSaveConfig({ preventDefault: () => {}, target: generalForm });
        } else {
            alert("Error: Campo no encontrado en el formulario general.");
        }
    } else {
        // Si no existe el formulario general, intentar guardar solo este campo
        handleSaveConfigField('whatsapp_number', numero);
    }
}
window.saveWhatsAppConfig = saveWhatsAppConfig;

// Función para guardar la configuración de puntos
function savePuntosConfig() {
    const puntosTexto = document.getElementById('puntos-texto')?.value.trim();
    const puntosRatio = parseFloat(document.getElementById('puntos-ratio')?.value || '0');
    const puntosMinimo = parseInt(document.getElementById('puntos-minimo')?.value || '0');
    const puntosDescuento = parseFloat(document.getElementById('puntos-descuento')?.value || '0');
    
    // Validación básica
    if (puntosRatio < 0 || puntosMinimo < 0 || puntosDescuento < 0) {
        alert("Los valores numéricos deben ser mayores o iguales a cero.");
        return;
    }
    
    // Usar el formulario general
    const generalForm = document.getElementById('config-general-form');
    if (generalForm) {
        if (generalForm.elements['benefits_description']) generalForm.elements['benefits_description'].value = puntosTexto || '';
        if (generalForm.elements['points_per_sol']) generalForm.elements['points_per_sol'].value = puntosRatio || '';
        if (generalForm.elements['points_min_redeem']) generalForm.elements['points_min_redeem'].value = puntosMinimo || '';
        if (generalForm.elements['points_discount_value']) generalForm.elements['points_discount_value'].value = puntosDescuento || '';
        ...
        const configToSave = {
            benefits_description: puntosTexto || null,
            points_per_sol: isNaN(puntosRatio) ? null : puntosRatio,
            points_min_redeem: isNaN(puntosMinimo) ? null : puntosMinimo,
            points_discount_value: isNaN(puntosDescuento) ? null : puntosDescuento
        };
        
        handleSaveMultiConfig(configToSave);
    }
}
window.savePuntosConfig = savePuntosConfig;

// Función para guardar un campo de configuración específico
async function handleSaveConfigField(fieldName, value) {
    if (!fieldName) return;
    ...
    try {
        ...
        console.error(`[gerente/saveConfigField] Error:`, error);
        showGlobalError(`Error al guardar configuración: ${error.message}`);
    }
}

// Función para guardar múltiples campos de configuración
async function handleSaveMultiConfig(configObj) {
    ...
    try {
        ...
        console.error(`[gerente/saveMultiConfig] Error:`, error);
        showGlobalError(`Error al guardar configuración: ${error.message}`);
    }
}
...
function addExplicitTabListener() {
    const empleadosTab = document.getElementById('tab-empleados');
    if (empleadosTab) {
        empleadosTab.addEventListener('click', function() {
            console.log('[gerente.js] Click en pestaña Empleados');
            loadGerenteUsers();
        });
    }
}
...
async function setupGerenteDashboard() {
    ...
    setupEventHandlers();
    setupTabHandlers();
    addExplicitTabListener();
    
    // Inicialmente cargar KPIs y verificar qué pestaña está activa
    try {
        ...
        // Verificar qué pestaña está activa al inicio
        const activeTab = document.querySelector('.tab-nav .active');
        if (activeTab) {
            const tabId = activeTab.id;
            console.log(`[gerente.js] Pestaña activa al inicio: ${tabId}`);
            
            // Cargar los datos según la pestaña activa
            if (tabId === 'tab-empleados') {
                loadGerenteUsers();
            } else if (tabId === 'tab-kpis') {
                // Esta es la carga predeterminada que ya tenías
                await Promise.all([
                    loadGerenteKPIs(),
            ...
            // Si no hay pestaña activa, cargar KPIs por defecto
            await Promise.all([
                loadGerenteKPIs(),
                setupMonthlySalesChart()
            ]);
...
export default {
   ...
   loadGerenteInventarioData,
   loadGerenteConfig,
   reloadGerenteInventarioData: loadGerenteInventarioData
};