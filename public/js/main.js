...
async function loadWarehousesIfNeeded(role) {
    ...
    try {
        ...
        if (response.status === 403) {
            // Si el rol (ej: repartidor) no tiene permiso para esta ruta, continuar sin almacenes
            console.log(`[main/loadWarehouses] Permiso denegado para ${role}, continuando sin almacenes.`);
            setCurrentWarehouses([]);
            setSelectedWarehouseId(getCurrentUser()?.default_warehouse_id || null);
            return true; // Consideramos esto un éxito para continuar con la carga de la app
        }
        ...
        // Incluso con error, establecemos valores vacíos para continuar con la carga
        setCurrentWarehouses([]);
        setSelectedWarehouseId(getCurrentUser()?.default_warehouse_id || null);
        
        // Mostrar error, pero solo si no es un problema de permisos que ya manejamos
        if (!error.message.includes('403')) {
            showGlobalError("Error al cargar la lista de almacenes: " + error.message);
        }
        
        // Para roles críticos que necesitan almacenes, consideramos esto un fallo
        return ['base', 'contabilidad', 'gerente'].includes(role);
    }
}
