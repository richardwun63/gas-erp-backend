...
// Exportamos la URL base de la API para que pueda ser usada por otros módulos que la necesiten
export { API_BASE_URL };
 ...
 */
const FETCH_TIMEOUT = 60000; // 60 segundos (aumentado para evitar timeouts)

/**
 * Número máximo de reintentos para peticiones fallidas por problemas de red
 */
const MAX_RETRIES = 3; // Aumentado a 3 reintentos
...
async function fetchApi(endpoint, options = {}, retries = MAX_RETRIES) {
    ...
    try {
        ...
        if (error.name === 'TypeError' || error.message.includes('excedió el tiempo')) {
            // Buscar función global para mostrar errores si está disponible
            if (typeof window.showGlobalError === 'function') {
                window.showGlobalError(`Error de conexión con el servidor. Por favor, verifica tu conexión a internet.`);
            } else {
