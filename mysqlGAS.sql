...
CREATE TABLE `cylindertypes` (
  ...
  CONSTRAINT `chk_price_exchange_positive_cyl` CHECK (`price_exchange` > 0), -- Cambiado el nombre para evitar duplicidad
  CONSTRAINT `chk_price_new_gt_exchange` CHECK (`price_new` > `price_exchange`) -- El precio nuevo debe ser mayor al de intercambio
