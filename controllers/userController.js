...
const listEmployees = async (req, res) => {
    const { 
        ...
        status, 
        search,
        excludeRole // Parámetro para excluir ciertos roles (ej: 'cliente')
    } = req.query;
    
    const offset = (page - 1) * limit;
    const requestingUserId = req.user.id;
    console.log(`GET /api/users (listEmployees) por User ID: ${requestingUserId}, Query:`, req.query);
    ...
    try {
        ...
        let sqlBase = `
            ...
            WHERE 1=1`; // Usamos 1=1 para facilitar la adición de condiciones
        
        const params = [];

        // Filtros adicionales
        if (role && role !== 'all') {
            sqlBase += ' AND r.role_name = ?';
            params.push(role);
        }
        ...
        if (excludeRole) {
            ...
            // Si no se especifica explícitamente excluir, por defecto excluimos clientes
            sqlBase += ' AND r.role_name != ?';
            params.push('cliente');
        }
        
        if (status && status !== 'all') {
            sqlBase += ' AND u.is_active = ?';
            params.push(status === 'active' ? 1 : 0);
        }
        
        if (search) {
            sqlBase += ' AND (u.full_name LIKE ? OR u.username LIKE ? OR u.email LIKE ? OR u.phone_number_primary LIKE ?)';
            const searchTerm = `%${search}%`;
            params.push(searchTerm, searchTerm, searchTerm, searchTerm);
        }

        // Ordenamiento
        const validSortColumns = ['user_id', 'username', 'full_name', 'role_name', 'is_active', 'created_at', 'updated_at'];
        const sortCol = validSortColumns.includes(sortBy) ? sortBy : 'full_name';
        const sortDir = sortOrder.toUpperCase() === 'DESC' ? 'DESC' : 'ASC';
        
        const tablePrefix = sortCol === 'role_name' ? 'r.' : 'u.';
        sqlBase += ` ORDER BY ${tablePrefix}${sortCol} ${sortDir}`;

        // Paginación
        const paginatedSql = `${sqlBase} LIMIT ? OFFSET ?`;
        params.push(parseInt(limit), parseInt(offset));

        console.log("SQL Employees:", paginatedSql, params);
        const [employees] = await connection.execute(paginatedSql, params);

        // Obtener total para paginación (sin limit/offset, con mismos filtros)
        // Reusamos los parámetros excepto limit y offset
        const countParams = params.slice(0, -2);
        const countSql = `SELECT COUNT(*) as total FROM Users u JOIN Roles r ON u.role_id = r.role_id WHERE 1=1`;
        
        // Reconstruir condiciones WHERE para la consulta de conteo
        let countWhere = '';
        if (role && role !== 'all') countWhere += ' AND r.role_name = ?';
        if (excludeRole) countWhere += ' AND r.role_name != ?';
        else countWhere += ' AND r.role_name != ?'; // Excluir clientes por defecto
        if (status && status !== 'all') countWhere += ' AND u.is_active = ?';
        if (search) countWhere += ' AND (u.full_name LIKE ? OR u.username LIKE ? OR u.email LIKE ? OR u.phone_number_primary LIKE ?)';
        
        const [totalRows] = await connection.execute(countSql + countWhere, countParams);
        const totalEmployees = totalRows[0].total;
