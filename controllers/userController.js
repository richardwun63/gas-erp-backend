...
const listEmployees = async (req, res) => {
    ...
    try {
        ...
        console.log("SQL Employees:", paginatedSql, paginatedParams);
        const [employees] = await connection.execute(paginatedSql, paginatedParams);

        // Obtener total para paginación (sin limit/offset, con mismos filtros)
        // Construir correctamente la consulta de conteo con las mismas condiciones
        const countSql = `
        ...
        // Reutilizar las mismas condiciones WHERE de la consulta principal
        let countWhere = '';
        
        if (role && role !== 'all') {
            countWhere += ' AND r.role_name = ?';
        }
        ...
        if (status && status !== 'all') {
            countWhere += ' AND u.is_active = ?';
        }
        
        if (search) {
            countWhere += ' AND (u.full_name LIKE ? OR u.username LIKE ? OR u.email LIKE ? OR u.phone_number_primary LIKE ?)';
        }
