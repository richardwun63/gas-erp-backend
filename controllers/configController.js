// controllers/configController.js (VERSIÓN CORREGIDA)
...
const getAllConfig = async (req, res) => {
    ...
    try {
        ...
        const configObject = configRows.reduce((acc, curr) => {
            ...
            try {
                if (curr.value_type === 'number') {
                    parsedValue = Number(curr.config_value);
                } else if (curr.value_type === 'boolean') {
                    parsedValue = curr.config_value === 'true' || curr.config_value === '1';
                } else if (curr.value_type === 'json' && curr.config_value) {
                    parsedValue = JSON.parse(curr.config_value);
                }
            } catch (e) {
                console.warn(`Error al parsear valor para ${curr.config_key}: ${e.message}`);
                // Mantener el valor original si falla el parsing
            }
            ...
            acc[curr.config_key] = { 
                ...
                valueType: curr.value_type || 'string'
            };
            return acc;
        }, {});
    ...
    try {
        ...
        for (const [key, value] of Object.entries(configUpdates)) {
            ...
            const updatePromise = connection.execute(
                `INSERT INTO Configuration (config_key, config_value, value_type, last_updated)
                 VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                 ON DUPLICATE KEY UPDATE 
                 config_value = VALUES(config_value),
                 value_type = VALUES(value_type),
                 last_updated = CURRENT_TIMESTAMP`,
                [key, valueToStore, valueType]
            )
            .then(([result]) => {
    ...
    try {
        ...
        const [result] = await connection.execute(
            `UPDATE Configuration 
             SET config_value = ?, value_type = ?, last_updated = CURRENT_TIMESTAMP
             WHERE config_key = ?`,
            [valueToStore, valueType, key]
        );
        ...
        if (result.affectedRows === 0) {
            await connection.execute(
                ...
                (config_key, config_value, value_type, last_updated)
                VALUES (?, ?, ?, CURRENT_TIMESTAMP)`,
                [key, valueToStore, valueType]
            );
        }
