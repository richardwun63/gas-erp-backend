// controllers/configController.js (CORREGIDO)
...
const getAllConfig = async (req, res) => {
    ...
    try {
        // SOLUCIÓN: Eliminamos la columna value_type que no existe en la estructura de la BD
...
const updateConfig = async (req, res) => {
    ...
    try {
        ...
        for (const [key, value] of Object.entries(configUpdates)) {
            ...
            // SOLUCIÓN: Usar INSERT ... ON DUPLICATE KEY UPDATE sin la columna value_type
            try {
...
const getConfigHistory = async (req, res) => {
    ...
    try {
        ...
        // Limitar número de resultados - CORRECCIÓN para MySQL 8.0.41
        if (limit && !isNaN(parseInt(limit, 10))) {
            const limitValue = parseInt(limit, 10);
            // En MySQL 8.0.41, no usamos placeholder para LIMIT
...
async function resetConfigValue(connection, key, defaultValue, userId, results) {
    try {
        ...
        // Actualizar en la base de datos - SOLUCIÓN: Eliminar la columna value_type
        ...
        // Si no existe, insertarla - SOLUCIÓN: Eliminar la columna value_type
